...
alter table `coupon_policy` change `expiration_time` `usage_period` INT not null;

alter table `coupon_policy` add `is_deleted` tinyint not null after `usage_period`;

alter table `issue_coupons` change `expiration_at` `expiration_date` date null;

alter table `coupon_type_percent` change `minimum_price` `minimum_order_price` INT unsigned not null;

alter table `coupon_type_percent` change `maximum_price` `maximum_discount_amount` INT unsigned not null;

alter table `coupon_type_cash` change `minimum_price` `minimum_order_price` INT unsigned not null;

alter table `oauth_accounts` modify column `account_code` VARCHAR(255) not null;

alter table `accounts` modify column `login_id` VARCHAR(255) not null;

alter table `account_address` add `renewal_at` datetime null after `alias`; -- 해당 테이블에 데이터가 존재하므로 null로 정의한 다음 renewal_at에 datetime 데이터들을 넣은 후 not null로 전환.

alter table `account_address` modify column `renewal_at` datetime not null;
alter table `stores` drop column `business_license`;

alter table `stores` drop column `image`;
...
alter table `stores` add `business_license_image_id` BIGINT NOT Null after `store_status_code`;
alter table `stores` add `store_image_id` BIGINT NOT NULL after `business_license_image_id`;
alter table `images` change `access_permission` `is_public` TINYINT not null;

alter table `stores` add foreign key(`business_license_image_id`) references `images`(`image_id`);

alter table `stores` add foreign key(`store_image_id`) references `images`(`image_id`);


alter table `menu` drop column `image`;
alter table `menu` add `menu_image_id` BIGINT not null after `store_id`;

alter table `menu` add foreign key(`menu_image_id`) references `images`(`image_id`);
...
CREATE TABLE IF NOT EXISTS  `cart_detail_menu_option` (
...
alter table `images` add column `origin_name` VARCHAR(255) not null after `image_id`;

alter table `issue_coupons` change `issue_at` `receipt_date` DATE null;

alter table `cart_details` add column `created_time_millis` BIGINT not null;

rename table `order_detail_menu_option` TO `order_detail_menu_options`;

rename table `cart_detail_menu_option` TO `cart_detail_menu_options`;

alter table `order_detail_menu_options` add column `now_name` VARCHAR(20) NOT NULL after `option_id`;

alter table `order_detail_menu_options` add column `now_price` INT NOT NULL after `now_name`;

alter table `order_details` add column `now_name` VARCHAR(30) not null after `count`;
